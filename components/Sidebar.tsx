...
export default function Sidebar({
  ...
  const wordlistId = params.wordlistId as string | undefined
  ...
  const wordlistCategoryId = wordlistId ? wordlistCategoryMap[wordlistId] : undefined
  ...
  const [confirmDialog, setConfirmDialog] = useState<{ show: boolean; href: string }>({ show: false, href: '' })
  ...
  // Scroll-spy only runs on the /vocab listing page
  useEffect(() => {
    if (!isVocabListingPage) return
    const handleScroll = () => {
      ...
      setScrollSpyCategoryId(currentId ?? (vocabCategories[0]?.id ?? null))
    }
    window.addEventListener('scroll', handleScroll, { passive: true })
    handleScroll()
    return () => window.removeEventListener('scroll', handleScroll)
  }, [isVocabListingPage, vocabCategories])

  // Determine which category ID is "active" in the sidebar
  useEffect(() => {
    if (isOnWordlistPage && wordlistCategoryId) {
      setActiveVocabId(wordlistCategoryId)
    } else if (isVocabListingPage) {
      setActiveVocabId(scrollSpyCategoryId)
    } else {
      setActiveVocabId(null)
    }
  }, [isOnWordlistPage, wordlistCategoryId, isVocabListingPage, scrollSpyCategoryId])
    ...
    <div className="flex flex-col h-full w-full bg-white">
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="w-[72px] shrink-0 border-r border-gray-200 bg-white flex flex-col items-center py-4 gap-2">
          {/* Grammar icon — uses confirmation if on wordlist page */}
          <button
            onClick={() => handleNavigateAway(grammarHref)}
            className={`w-14 items-center justify-center p-2 rounded-xl flex flex-col gap-1 transition-colors ${isGrammarIconActive ? 'bg-[#EFF3FC] text-[#221B2F]' : 'text-gray-500 hover:bg-gray-50'}`}
          >
          ...
          {/* Vocab icon — uses confirmation if on wordlist page and navigating to /vocab */}
          <button
                ...
                {vocabCategories.map(cat => {
                  ...
                  return (
                    <button
                      ...
                      onClick={() => {
                        if (isOnWordlistPage) {
                          // Navigate back to /vocab and scroll to that category
                          router.push(`/vocab#category-${cat.id}`)
                        } else {
    ...
    <>
      ...
      {confirmDialog.show && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center">
          ...
          />
          {/* Dialog */}
          <div className="relative bg-white rounded-2xl shadow-2xl p-6 w-[340px] mx-4 animate-[fadeInScale_0.2s_ease-out]">
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                <h2 className="font-bold text-gray-900 text-[15px]">Leave this page?</h2>
                <p className="text-[13px] text-gray-500 mt-0.5">You'll be redirected away from this wordlist.</p>
              </div>
            </div>
            <div className="flex gap-2 mt-5">
              ...
              <button
                ...
                className="flex-1 rounded-lg bg-[#221B2F] px-4 py-2.5 text-sm font-semibold text-white hover:bg-[#2e2440] transition-colors"
              >
                Yes, leave
              </button>
            </div>
