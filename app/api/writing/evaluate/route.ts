...
export async function POST(request: Request) {
  try {
    ...
    const { task_prompt, paragraph_type, user_answer, hint_vocabularies } = body

    if (!task_prompt || !paragraph_type || !user_answer || !Array.isArray(hint_vocabularies)) {
      return NextResponse.json({ error: 'Invalid payload' }, { status: 400 })
    }

    // Fetch AI config
    let config = await prisma.aiConfig.findFirst({
      where: { type: 'writing-paragraph' }
    })

    // Fallback to vocab-paraphrase if writing-paragraph is not yet created
    if (!config) {
    ...
    const systemPrompt = config.globalPrompt || 'You are an IELTS writing evaluator evaluating a specific paragraph.'
...
`
    ...
    console.error('Error evaluating writing paragraph:', error)
